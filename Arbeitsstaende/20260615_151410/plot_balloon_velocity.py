...
scale = 2
img = Image.new("RGB", (width * scale, height * scale), "#f7f5ef")
...
draw.text(
    (80 * scale, 113 * scale),
    "Modell mit zwei Richtungswechseln und fast ausgeglichener Flaechenbilanz",
    fill="#596166",
    font=subtitle_font,
)
...
line([(px(0), py(y_min)), (px(0), py(y_max))], "#3f474b", 2)
line([(px(x_min), py(0)), (px(x_max), py(0))], "#3f474b", 2)
...
draw.polygon([(int(x), int(y)) for x, y in positive_poly], fill="#cfe8dd")
...
draw.polygon([(int(x), int(y)) for x, y in negative_poly], fill="#f3c9c0")
...
formula = f"v(t) = {k:.3f} * t * (t - {r:.1f}) * (t - {s:.3f})"
draw.rounded_rectangle(
...
draw.text((1160 * scale, 895 * scale), f"Endabstand: {antiderivative(7):.3f} km", fill="#27434b", font=small_font)

draw.text((195 * scale, 835 * scale), "Gruen: Fahrt in positive Richtung", fill="#3b6f5c", font=small_font)
draw.text((195 * scale, 872 * scale), "Rot: Fahrt in Gegenrichtung", fill="#99504a", font=small_font)
draw.text((195 * scale, 909 * scale), "Die Flaechen sind fast gleich gross: Landung nahe am Start.", fill="#596166", font=small_font)
